...
void* __wrapperFunc(void* arg){
	...
	sigwait(&set, &retSig);
	fprintf(stderr, "child thread received signal and exit sigwait (__wrapperFunc)\n");
	...
	fprintf(stderr, "child thread will call function pointer (__wrapperFunc)\n");
...
Thread* __getThread(thread_t tid){
	...
	fprintf(stderr, "TCB isn't in ready or wait queue (__getThread)\n");
	return NULL;
